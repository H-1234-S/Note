...
>  `render` 函数中返回的元素(也就是 `wipRoot` )，与之前提交到 DOM 中的那个 fiber 树**进行比较**。
...
- 在 React 中还使用了 key 这一机制，可以看成 diff 算法中的节点身份机制(注意只有 key 和 type 都相同，才认为这是同一个节点)
	...
	- 不加 key，React只能按位置比较，fiber 节点只是换了一个位置，例如从 `A B C` 换成 `C A B`，React 会对所有节点重新计算
	- 加了 key，React 先建立映射，`A B C` 三个节点都可以在映射中找到， 将之前位置的 fiber 节点移动到新位置即可，这不仅是**性能优化**，还保持**组件状态**
...
function updateDom(dom, prevProps, nextProps) {
	...
	Object.keys(prevProps)
		.filter(isEvent)
		.filter(
			// nextProps 中没有该 key 或 key 已经更改了的
			key =>
				!(key in nextProps) ||
...
这时react中有类requestIdleCallback运行机制，在浏览器中每一帧的空闲时间插入wookloop函数

performUnitOfWork函数处理fiber节点
	先判断是否为函数节点，如果为函数节点则运行函数，得到函数节点的子元素
...
```
React 的 diff 主要是对同一位置新旧两种元素的比较

怎么拿到oldFiber tree？

在commit工作结束后，保存对当前 fiber tree 的引用

在下一次构建工作开始时，wiproot的alternate指向上一次的currentRoot

diff 差异化更新是怎么实现的？

主要是三种情况：
	如果类型相同，那么更新props即可
	如果类型不同或者旧元素不存在，那么就需要创建对应的fiber节点
	如果旧节点存在，新节点不存在，则删除oldfiber节点
	
	同时react还在diff中添加了key这一确认节点身份机制
```
## 为什么需要 Fiber 架构？
...
- **Render 阶段**：通过调和算法标记每个节点的 `effectTag`。这个过程在内存中完成，可以被中断，用户感知不到。
    
- **Commit 阶段**：一旦 `wipRoot` 构建完成，一气呵成地执行 `commitRoot`。
...
可能还会有兄弟节点，因此node.sibling如果存在接着删
```
...
如果删除的节点是函数式节点呢？还需要往下遍历，找到第一个具有dom节点的子节点，进行删除
...
```
实现一个useState基本逻辑：
返回一个数组，第一个元素是当前的状态，第二个元素是set更新函数
如果有旧状态就使用旧状态，没有就使用初始值
调用set更新函数时，并不立即进行action，而是放入一个队列中
在下一次渲染前执行该action队列
之后触发该组件的重新渲染
```
...
- 因为 React 依赖 `hookIndex` 的**自增顺序**来匹配状态，也就是通过**调用顺序**识别当前运行的是哪个useState函数。
...
下一次 useState 执行时会清空队列，这就是“批处理”的本质
...
react 内部的调度机制决定执行该**组件**,调用该组件得到返回的 JS 对象,也就是虚拟 DOM

接着往下处理,创建对应的 Fiber 节点和进行**调和处理**,也就是 Diff，计算完毕进行 commit 提交渲染