...
* setup：处理 Effect 的函数。setup 函数选择性返回一个 **清理（cleanup）** 函数。当组件被添加到 DOM 的时候，React 将运行 setup 函数。

* 在每次依赖项变更重新渲染后，React 将首先使用**旧值**运行 cleanup 函数（如果你提供了该函数），然后使用新值运行 setup 函数。在组件从 DOM 中移除后，React 将最后一次运行 cleanup 函数。
...
> startTransition 是同步执行的，只是给打了一个标签，为低优先级渲染，告诉 react 空闲时再进行渲染
### 语法
...
* 例如搜索框和搜索结果联动，没有useDeferredValue时每输入一个字都要更新；使用后
