...
| ----------- | ------------------ | ----------------- |
| `'string'`  | 字符串                | `"hello"`         |
| `'number'`  | 任意数字（含小数）          | `3`、`3.14`、`-0.5` |
| `'integer'` | **整数**（number 的子集） | `3`、`-10`、`0`     |
...
在 Fastify 中一切皆插件。使用 `register` API 挂载插件。这是添加路由、插件等的唯一途径；每一个插件有一个独立的作用域。

Fastify 按照声明的顺序加载插件，只有在当前插件加载完毕后才会加载下一个插件。因此可以在第一个插件中注册数据库连接器，在第二个插件中使用
...
- **熔断技术**：当后端服务出现故障或异常时，网关层可以使用熔断技术来防止请求继续发送到出现问题的服务上。通过监控后端服务的状态和性能指标，网关层可以**自动切换到备用服务或返回错误响应**，以提高系统的容错性和可靠性。
...
那么微服务指的就是，将应用程序拆分成为一系列小型、独立的服务，每个服务都是专注于执行特定的业务。

比如文章的服务就执行，文章的逻辑；用户的服务，就执行用户的逻辑

这些服务可以独立开发，测试，部署以及扩展，并且可以通讯。