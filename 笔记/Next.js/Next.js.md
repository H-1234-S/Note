...
- 原理：当它被调用时，它会抛出一个 Next.js 内部错误，Next.js 会捕获这个错误并告诉浏览器：“别加载这个页面了，直接去另一个 URL”。
...
**解决什么问题？**
	对于传统SSR，服务器解析生成html发给浏览器后，还会将所有的js发给浏览器进行水合
	即使某些页面只用于展示数据，不需要水合，它们js也会被下载执行
...
第一次构建（build）时，Next.js 会自动将**静态路由**渲染成 HTML 和 RSC Payload 并存储。

当下次请求命中缓存时，直接返回缓存结果，目的是减少服务器计算压力，实现类似于静态网站的响应速度。
