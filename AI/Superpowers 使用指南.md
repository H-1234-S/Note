...
测试驱动开发
 - 也就是 TDD 驱动原则，强制写代码前先写测试用例，根据测试用例进行代码编写

 - superpowers 会先实现一个失败的测试用例，再实现一个恰好通过的测试用例

微型计划与任务追踪
 -  当一个需求过于庞大时，ai可能会执行几个小时

 -  superpowers 会将该任务拆成原子级的任务进行实现

 -  也就是将几个小时的任务拆解为几分钟的原子级任务

Git 工作树隔离
 - 只要启动任务，Superpowers 就会在底层自动利用 `git worktree` 或 `git branch` 为这个特定的任务开辟一个**完全独立的、干净的隔离沙盒空间**。

 -  因此本地分支是干净的，只有TDD测试全部通过时才会安全的合并到主分支

系统化调试
 - 当子功能与子功能之间联调时遇到bug时用到该功能，传统的ai习惯猜错误出现在哪里

 - Superpowers 的调试功能强制 AI 必须执行四步法：
	 - **复现（写一个必错的测试） -> 隔离（找根本原因） -> 修复 -> 验证**。
