...
同步功能的流程可以概括为：useQuery查询了某一个数据表示**订阅某个 Query Function 的结果**。那么使用useMutation**导致该Quyer Function的结果发生变化**后(使用 mutation 导致查询范围内的字段发生变化时)，useQuery自动运行，然后React刷新ui
...
主要解决的是在**serverless环境**下如何可靠执行后台任务，也就是长任务执行、多步骤流程、定时、重试还有异步任务
...
```
例如谷歌浏览器，有浏览器进程也就是主进程，负责浏览器主页面展示和处理用户交互
...
```
当打开一个项目时，会有项目的具体id，使用useQuery获取该id下的所有的文件数据

得到的是一个扁平的文件数组，需要转为嵌套结构的文件树

先建立一套映射结构，也就是使用map，键是文件id，值是文件数据形成的对象

通过遍历文件数组，对其中的每一个文件进行处理

通过遍历parentId，得到一个完整的文件路径组成的数组，这样最后一个元素要么是文件、要么是文件夹，但是之前的元素一定是文件夹

这样只需要对最后一个文件类型进行判断，根据是文件还是文件夹进行不同的处理
...
会根据内容进行计算，得到一个唯一的**哈希值**，这个哈希值就是该文件的唯一 ID。
...
拿到仓库内“文件结构快照”，按照**文件结构**创建文件夹，因为文件肯定是放在文件夹里的，先创建好文件夹才可以放对应的文件，之后按照 `SHA` 获取对应文件内容，解析响应的数据，创建文件。

**注意：** 

文件内容是按base64数据格式从github发送到客户端的，解析为原始字节码后可能对应的是二进制数据也可能对应的是文本数据，进行不一样的处理，对于二进制数据存储到convex的storage中

---

> 用户点击“导入 GitHub 仓库”以后，从 OAuth 授权、拿 token、列仓库、选择仓库，到把 GitHub 仓库内容写入 Convex 虚拟文件树，完整流程是什么？

**运行流程：**
```
用户输入github仓库url，点击确认后

向 /api/github/import 发送请求

在 app router 中首先验证用户身份，获取github token

之后触发inngset后台任务，把github import 任务交给inngset后台可靠执行，返回Success

------------------------------------------------------

在inngset中，初始化 octokit，调用 octokit.rest.git.getTree 递归获取仓库文件树

按照type是blob还是tree进行不同的处理

先得到所有的文件夹，并且对文件夹进行深度排序

将 path按照 / 分割转为数组，判断数组长度，如果长排在后面，这样确保创建子文件夹时父文件夹存在
...
如果是图片数据，先从convex请求一个临时的url用于上传二进制数据，再根据上传成功的id在数据库中创建一个记录
...
这其实是git对象模型了，创建文件内容对象(blob) -> 挂载到目录结构上(tree) -> commit提交(commit) -> 更改主分支引用(ref)
