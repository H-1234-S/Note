...
在这个过程中需要根据不同的状态渲染不同的ui；错误状态，那么就展示错误信息；录制中，那么就展示波形图；音频数据收集完毕，展示对应的音频文件
...
>用户录制一段声音样本用于声音克隆时，从浏览器获取麦克风权限开始，到得到可上传的音频文件，这条前端音频链路怎么实现？
>包括权限、MediaStream、录制格式、波形渲染、停止录制、资源释放。
...
之后初始化 RecordRTC，接入 MediaStream 实例对象，用于收集音频数据，调用recorder.startRecording方法开始录音

更改状态 isRecording 为 true，前端渲染对应ui

在此过程中，使用try、catch捕获错误，如果获取麦克风权限失败，则将错误在前端渲染

当状态 isRecording 为 true，也就是画布div已经在前端展示，那么初始化ws实例，也初始化Record插件，接入媒体流，使用useRef保存对应实例，方便后续释放资源

当用户点击暂停时，也就是调用stopRecording函数

在函数中调用recorder.stopRecording方法暂停录音，调用recorder.getBlob方法获取blob对象，更改对应的状态，释放所有资源

之后使用 new File([blob], "recording.wav", { type: "audio/wav",}) 转为 file 对象
