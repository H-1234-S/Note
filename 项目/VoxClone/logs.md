...
在这个过程中需要根据不同的状态渲染不同的ui；例如错误状态，那么就展示错误信息；例如录制中，那么就展示画布，也就是波形图；例如音频数据收集完毕，那么就展示对应的音频文件
...
用户录制一段声音样本用于声音克隆时，从浏览器获取麦克风权限开始，到得到可上传的音频文件，这条前端音频链路怎么实现？包括权限、MediaStream、录制格式、波形渲染、停止录制、资源释放。
