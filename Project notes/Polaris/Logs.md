...
**核心概念：**`immutable state`和`transaction`，将编辑器的状态视为不可变的，每次更改返回新的状态，这与react很类似。`transaction`就是状态变化的“描述对象”，也就是本次操作所产生的信息包，`immutable state`和`transaction`一起生成`new state`
