...
github import 思路：拿到仓库内“文件结构快照”，按照**文件结构**创建文件夹，因为文件肯定是放在文件夹里的，先创建好文件夹才可以放对应的文件，之后按照 `SHA` 获取对应文件内容，解析响应的数据，创建文件。

**注意：** 文件内容是按base64数据格式从github发送到客户端的，解析为原始字节码后可能对应的是二进制数据也可能对应的是文本数据，进行不一样的处理，对于二进制数据存储到convex的storage中
