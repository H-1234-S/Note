...
**注意：** JS在执行流程分为两步；第一步是预编译阶段，该阶段收集执行上下文；第二步是执行阶段
...
JS 执行函数时会创建函数执行上下文并压入调用栈，后进先出。函数执行完，上下文通常会弹出。但如果内部函数被外部引用，内部函数的 `[[Environment]]` 仍然指向定义时的词法环境，相关变量仍然可达，所以不会被垃圾回收。

这就是闭包不会被回收的原因：不是“闭包特殊”，而是“还有引用链能访问到变量”。垃圾回收基于可达性，只要变量环境仍可从根对象访问，它就不能释放。

变量提升：

1. `var` 声明会被提升并初始化为 `undefined`。
2. 函数声明会整体提升。
3. `let` / `const` 也会被提升，但在声明语句执行前处于暂时性死区，不能访问。
