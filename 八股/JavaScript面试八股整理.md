...
## 2. 执行上下文、作用域、闭包与 this、词法环境、词法作用域

### 2.1 概念解释

执行上下文是 JS 代码运行时的环境。每进入一段可执行代码，例如全局代码、函数代码、模块代码，都会创建对应执行上下文。
...
**注意：** 进入**全局代码**或**函数调用**时会**创建执行上下文**，并在创建阶段建立词法环境、变量环境、this 绑定等

作用域决定“变量在哪里可访问”，作用域链决定“找变量时按什么路径向外查找”。
...
词法作用域是一套规则，指变量的访问权限在书写的时候就确立了

词法环境是规则里运行的结构，例如：词法作用域定义之后的内部每一个**变量的信息**
	包括**环境记录**和**外部词法环境引用**
	**外部词法环境**引用意思是指向另一个**词法环境**，这就是**作用域链的本质**
...
> 函数身上有个属性 fn.[[Environment]]，里面存了一个指针，指向该函数定义时所处的那个词法环境
...
2. 函数声明会整体提升。
3. `let` / `const` 也会被提升，但在**声明语句执行前**处于**暂时性死区**，不能访问。
...
> 微任务用于在当前同步代码结束后、浏览器渲染和下一个宏任务之前，尽快执行一些需要保持状态一致性的逻辑。例如 Promise 状态传递、MutationObserver DOM 变化通知。如果全部放进宏任务，状态更新会被延迟到下一轮事件循环，可能产生可见的不一致。
...
> 微任务用于在当前任务结束后立即完成状态收敛，例如 Promise 链式传值和 DOM 变更通知。它必须早于下一个宏任务和渲染，否则异步状态可能延迟到下一轮，造成逻辑不一致。
...
浅拷贝只复制第一层属性。如果属性值是对象，复制的是引用。深拷贝会递归复制对象内部结构，使新旧对象互不影响。

防抖和节流：

| 技术 | 含义 | 场景 |
|---|---|---|
| 防抖 debounce | 事件停止触发一段时间后再执行 | 搜索联想、窗口 resize |
| 节流 throttle | 固定时间间隔内最多执行一次 | 滚动监听、拖拽、按钮连点 |
