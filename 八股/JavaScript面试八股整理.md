...
其实函数本质上也是对象，因此既有 `prototype` 也有 `__proto__`
...
`Promise.race()` 多个 Promise 执行，谁最先确定状态（fulfilled 或 rejected），就采用谁的结果，**忽略后续 Promise 执行结果**。

`Promise.any` 多个 Promise 执行，第一个状态变为 fulfilled 结果
...
function deepClone(value, cache = new WeakMap()) {
  ...
  if (value instanceof RegExp) {
	  // 提取原正则的文本（.source）和修饰符（.flags，如 g, i, m）
...
频繁触发事件，只执行最后一次。但是防抖有个副作用，对于第一次执行也需要延时执行
...
1. **第 0 秒**：用户第一次点击。`remaining <= 0`，立刻执行（走第一个 `if`）。此时 `lastTime = 0秒`。
    
2. **第 0.5 秒**：用户第二次点击。还在 CD 内，进不去第一个 `if`。来到第二个 `if`，发现没人在排队（`!timer`），于是启动一个定时器，等剩下的 `0.5秒`（`remaining`）。
    
3. **第 0.7 秒**：用户第三次点击。还在 CD 内，进不去第一个 `if`。来到第二个 `if`，发现**已经有人在排队了**，直接无视。
    
4. **第 1.0 秒**：时间到了！定时器爆炸，执行函数。同时 `lastTime` 被更新为 `1.0秒`，`timer` 被清空为 `null`。
    
5. **第 1.1 秒**：用户又点了一下。此时距离上次执行（1.0秒）只过去了 0.1 秒，依然在 CD 内。进不去第一个 `if`，来到第二个 `if`，发现没人在排队，再次启动一个定时器，等剩下的 `0.9秒`……
...
JS 的垃圾回收机制主要**基于可达性**：从根对象出发能访问到的对象是存活对象，访问不到的对象可以被回收。

常见根对象：

1. 全局对象。
2. 当前调用栈中的局部变量。
	意思是：调用栈中正在运行的函数代码，**其中定义的变量**随时可能被后续代码使用，因此这些变量直接被视为根。
	
3. 闭包引用。
4. DOM 引用。
	意思是：JavaScript 代码中对 HTML 元素的引用（例如通过 `document.getElementById('btn')` 获取到的对象）
	只要一个 DOM 节点当前正**处于浏览器的 DOM 树中**，浏览器内核就会强引用它，它就被视为一个 GC Root。
	
5. 原生模块或宿主环境持有的引用。
	意思是：JS由V8引擎解析执行，V8是用C++写的，由 C++ 底层直接持有并传递给 JavaScript 的对象，或者运行时自身需要的核心组件。

### 8.2 底层原理

在 JavaScript（V8 引擎）中，垃圾回收的核心算法是 **标记-清除（Mark-and-Sweep）**，其根本逻辑就是基于**可达性**。

虽然有了“可达性”这个标准，但如果每一次垃圾回收，引擎都要把内存里所有的对象从头到尾扫描一遍（检查可达性），**代价实在是太大了**，会导致网页卡顿（Stop-The-World）。
...
2. Ignition 解释器生成并执行字节码。
	**生成字节码：** Ignition 会把 AST 编译成字节码，字节码**省内存**和**跨平台**执行
	**执行与收集信息：** Ignition 开始逐行解释执行这些字节码。在执行的过程中，Ignition 它会记录代码运行时的信息。比如：某个函数被调用了多少次？传进来的参数是什么类型的（是数字还是字符串）？
	
3. 热点函数交给 TurboFan 优化编译为机器码。
	**热点函数：** 也就是执行了很多次的函数，会被标记为热点函数
	**TurboFan：** TurboFan会把热点函数的字节码拿过来，结合 ignition 里收集到的**类型假设信息**（比如：“根据之前执行了 1000 次的经验，这个 `add(a, b)` 函数的 `a` 和 `b` 都是数字”），直接把字节码编译成极其高效的**底层机器码（Machine Code）**
	
4. 如果**类型假设失效**，发生反优化，退回字节码。
	因为JS是一门**动态类型语言**，变量的类型可能会改变，那意味着Ignition收集的信息可能是错误的
	因此从机器码到字节码会发生反优化
	也就是对于生成的**机器码**，V8 丢弃这段已经生成的机器码，把执行权重新交还给 **Ignition 解释器**，退回到安全的**字节码状态**
...
1. Map 保持插入顺序，对于最久没动过的数据在最前面，新插入的数据在最后面。
...
意思是：在编译时期浏览器内部给每个模块创建的对象；用来记录、维护一些信息
...
如果之后浏览器再遇到相同导入，直接找到已有 Module Record。不会重新解析。
