...
`layout.tsx` 里没有登录就`redirect("/sign-in")`，这只是在ui层做了鉴权，但是重要的不是页面而是数据，鉴权的目的是为了保护数据，Next中有三种服务端获取数据逻辑

Server Component 渲染时会执行异步代码获取数据，但是当页面在`layout.tsx` 保护之外怎么办？因此需要鉴权，其实只要获取数据就应该鉴权
