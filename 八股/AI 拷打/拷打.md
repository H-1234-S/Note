...
> 打开 Chrome Performance，也就是查看火焰图

查看是不是网络导致的“卡”，有可能一直等接口返回数据

看有没有长任务执行、看JS执行时间长；浏览器16.6ms刷新一次屏幕，如果存在长任务可能会导致浏览器不响应

查看时间花在哪里？是JS计算太久？是React Render计算很长时间？是Layout很长？是Paint很长？

> 排查React

是否一个 state 放得太高，导致整棵子树都重新计算。虽然不一定会导致重新渲染

列表项是否缺少稳定的 `key`，导致 React 无法复用已有节点。

是否传递了新的对象、数组、函数引用，导致 `React.memo` 失效。

Context 的值是否频繁变化，影响大量消费者。

是否一次渲染了几千条数据，应该使用虚拟列表。
...
react 19 更新重点主要放在了 `action`，也就是异步操作上

`action` 任何提交数据的操作都可以称作为 `action`

`useActionState` 用于 `action` 操作与前端的交互；例如登录失败提示操作

`useOptimistic` 用于更乐观更新用户界面；适用于所“先操作ui，再发送请求场景”
