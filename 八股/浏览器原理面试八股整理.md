...
浏览器渲染流程要解决的问题是：如何把网络拿到的 HTML、CSS、JS、图片、字体等资源，变成屏幕上一帧一帧的像素。

这件事比“把 HTML 画出来”复杂得多。因为浏览器必须同时满足几个要求：

1. 边下载边解析，不能等所有资源都下载完才开始工作。
2. JS 可以随时修改 DOM 和样式，渲染结果必须和 JS 语义一致。
3. CSS 选择器、继承、层叠、媒体查询会共同决定最终样式。
4. 页面可能很大，但用户只看视口附近，浏览器要尽量增量更新。
5. 60Hz 屏幕下一帧只有约 16.7ms，渲染必须可拆分、可并行、可复用。

因此浏览器把“从文档到像素”的过程拆成多个阶段：解析、样式、布局、绘制、光栅化、合成。拆阶段不是为了让概念更复杂，而是为了让浏览器能在不同变化下只做必要的工作。

面试可以先这样答：

> 浏览器渲染流程是把 HTML、CSS 和 JS 共同决定的页面状态转换为屏幕像素的过程。大致流程是：HTML 解析生成 DOM，CSS 解析生成 CSSOM，DOM 和 CSSOM 结合形成可渲染的结构；浏览器进行样式计算、布局，确定每个盒子的几何信息；再生成绘制指令，经过分层、光栅化和合成，最终显示到屏幕。JS、CSS、图片、字体都会在不同阶段影响这个流程。
...
```mermaid
flowchart TD
    A[HTML 字节流] --> B[解码与 Token 化]
    B --> C[构建 DOM Tree]
    D[CSS 文件 / style / 行内样式] --> E[构建 CSSOM]
    C --> F[样式计算 Style]
    E --> F
    F --> G[布局 Layout]
    G --> H[绘制 Paint]
...
注意：真实浏览器实现并不一定严格按这张图一次性线性执行。它会边下载边解析，遇到脚本可能暂停，样式未准备好会阻塞渲染，后续 DOM 或 CSS 变化也会触发局部重算。面试时可以用这条链路建立整体模型，再补充阻塞和增量更新。

### 3.3 HTML 解析：DOM 是怎么来的

HTML 不是一开始就是 DOM。浏览器拿到的是网络字节流，需要经过解码和解析：
...
DOM 的作用不是“显示页面”，而是表示文档结构，并提供 JS 可操作的对象模型。后续样式计算、布局、事件分发、可访问性树等都依赖 DOM。
...
<div id="root"></div>
<script>
  document.getElementById("root").textContent = "changed";
...
所以浏览器遇到普通同步 `<script>` 时，通常要暂停 HTML 解析，先执行脚本，再继续向后解析。否则脚本看到的 DOM 状态、`document.write` 的插入位置、执行顺序都可能不符合 Web 规范。

这就是为什么大 JS 包会影响首屏：不仅下载需要时间，执行也会占用主线程，推迟 DOM 构建和后续渲染。

### 3.4 CSS 解析：CSSOM 为什么会阻塞渲染

CSS 会被解析为 CSSOM。CSSOM 不是简单的字符串列表，而是浏览器理解后的样式规则结构，包含选择器、属性、继承、层叠、媒体查询等信息。
...
浏览器需要结合 DOM 和 CSSOM 才能知道每个元素最终样式：
...
继承 body 的 font-size
  ↓
得到 color: red; font-size: 16px; display: block; ...
```

CSS 通常不阻塞 DOM 解析。浏览器可以一边下载 CSS，一边继续解析 HTML。但 CSS 会阻塞渲染，因为没有最终样式就无法可靠地构建渲染结果。

如果 CSS 没加载完就先画页面，会出现两个问题：

1. 用户可能先看到无样式页面，再突然变成有样式页面，也就是 FOUC。
2. 样式到达后大量布局和绘制要重做，反而浪费更多工作。

所以浏览器通常会等待关键 CSS 准备好，再进行首次渲染。

#### CSS 为什么可能阻塞 JS 执行

CSS 本身不执行代码，但 JS 可能读取样式和布局：
...
如果脚本执行时 CSS 还没加载完成，`offsetWidth` 的结果就不可靠。因此当同步 JS 位于尚未加载完成的阻塞样式表之后时，浏览器可能等待 CSSOM 构建完成，再执行 JS。
...
这就是为什么“CSS 不阻塞 DOM 解析”这句话只说了一半。更完整的说法是：CSS 不直接阻塞 HTML 解析，但会阻塞渲染；并且在某些同步脚本场景下，会间接阻塞 HTML 解析。

### 3.5 样式计算：从规则到每个元素的最终样式

样式计算的目标是为每个需要渲染的 DOM 节点计算最终样式，也就是 computed style。
...
浏览器要判断 `.card p` 和 `p` 都匹配，但 `.card p` 优先级更高，所以最终 `color` 是 `#333`，`font-size` 继承自 `body`。
...
### 3.6 Render Tree：为什么 DOM 不等于渲染树

DOM 表示文档结构，但不是所有 DOM 节点都会显示。浏览器会基于 DOM 和最终样式生成用于渲染的结构。很多资料称它为 Render Tree；在不同引擎中实际内部结构名称会不同，但面试中可以用这个概念表达“可见渲染对象树”。

几个典型区别：

| 节点或样式 | 是否参与渲染 | 原因 |
| --- | --- | --- |
| `display: none` | 不参与布局和绘制 | 元素不生成盒子 |
| `visibility: hidden` | 参与布局，不可见 | 盒子占位，但绘制时隐藏 |
| `opacity: 0` | 参与布局，可能参与合成 | 透明但仍占位，也可能响应事件 |
| `<head>`、`<script>` | 通常不生成可见盒子 | 它们不是视觉内容 |
| 伪元素 `::before` | 可能参与渲染 | 它不是 DOM 节点，但会生成渲染内容 |

这解释了一个常见问题：DOM Tree 和 Render Tree 不是一一对应的。一个 DOM 节点可能不生成渲染对象，一个 DOM 节点也可能生成多个盒子，伪元素还可能生成额外的渲染内容。

### 3.7 Layout：浏览器如何计算位置和尺寸

Layout，也常被称为 Reflow，目标是计算每个盒子在页面中的几何信息：

- 宽高是多少。
- 在父元素中的位置在哪里。
- 行内文本如何换行。
- 百分比、`auto`、`flex`、`grid` 如何求值。
- 图片、字体、滚动条是否影响尺寸。
...
浏览器必须先知道 `.container` 的宽度是 `300px`，再根据 flex 规则计算两个 `.item` 各自的宽度。真实页面里，父子、兄弟、字体、图片、滚动条都会互相影响，所以 Layout 往往是渲染链路中比较昂贵的阶段。

会触发布局的常见变化：

- 修改元素尺寸：`width`、`height`、`padding`、`border`。
- 修改位置：`top`、`left`、`margin`。
- 改变文档流：`display`、`position`、`float`。
- 修改内容：文本变化、插入删除 DOM。
- 字体加载完成后文字尺寸变化。
- 读取布局属性时触发强制同步布局，例如 `offsetWidth`、`getBoundingClientRect()`。

#### 强制同步布局为什么危险

浏览器通常会把 DOM 写操作先标记为“布局脏了”，等到合适时机再统一计算。但如果 JS 立刻读取布局信息，浏览器必须提前把布局算出来。

```js
box.style.width = "200px";      // 写：标记布局失效
console.log(box.offsetWidth);   // 读：强制浏览器立刻 Layout
box.style.height = "200px";     // 又写
console.log(box.offsetHeight);  // 又读：再次强制 Layout
```
...
这也是很多动画库、虚拟列表、React 性能优化会强调“减少 layout thrashing”的原因。

### 3.8 Paint：绘制不是直接上屏

Layout 确定几何信息后，浏览器进入 Paint。Paint 的目标不是立刻把像素写到屏幕，而是生成绘制指令。
...
元素大小和位置没变，所以一般不需要重新 Layout，但视觉内容变了，需要重新 Paint。
...
### 3.9 Layer、Raster、Composite：从绘制指令到最终帧

现代浏览器不会把整个页面当成一张大图每次重画。它会把页面拆成多个图层，再分别光栅化和合成。
...
- 3D transform。
- `position: fixed` 或某些滚动相关元素。
- video、canvas。
- opacity 或 transform 动画。
- `will-change` 提示。
- 浏览器内部判断需要独立合成的元素。

分层的好处是局部变化时可以复用其他图层。例如一个弹窗移动，只需要重新合成弹窗图层，不一定重绘整个页面。

但图层不是越多越好。每个图层都需要内存，过多图层会增加管理、光栅化和合成成本。

#### 光栅化 Raster

Paint 生成的是绘制指令，Raster 负责把这些指令转成位图或 GPU 纹理。
...
切块的原因是页面可能非常大，浏览器不必一次性光栅化整个页面。它可以优先处理视口附近的 tile，提高首屏和滚动体验。

#### 合成 Composite

Composite 负责把多个图层按照位置、变换、透明度、裁剪关系合成为最终帧。
...
浏览器可能跳过 Layout 和 Paint，只更新合成参数：
...
这就是 `transform` 和 `opacity` 常被推荐用于动画的原因：它们更容易走合成路径，减少主线程和 Paint 压力。

### 3.10 资源阻塞关系：CSS、JS、图片、字体分别影响哪里

#### CSS：阻塞渲染，可能间接阻塞 HTML 解析

```html
<link rel="stylesheet" href="/style.css">
```

CSS 会阻塞首次渲染，因为浏览器需要 CSSOM 来计算最终样式。同步 JS 如果位于阻塞 CSS 之后，并且可能读取样式，也可能等待 CSS 加载完成。
...
普通同步脚本会阻塞 HTML 解析。脚本下载、解析、编译、执行都会推迟后续 DOM 构建和渲染。

优化方向：

- 使用 `defer` 让脚本在 DOM 解析后执行。
- 使用 `async` 加载独立脚本。
- 拆分长任务。
- 减少首屏 JS。
- 避免首屏同步执行过多 React 逻辑。

#### 图片：通常不阻塞 DOM 解析，但影响布局和 LCP

图片下载一般不阻塞 DOM 构建，但图片尺寸会影响布局。如果浏览器不知道图片宽高，图片加载完成后可能把页面撑开，造成 CLS。

```html
<img src="/hero.jpg" width="1200" height="600" alt="">
```

设置 `width`、`height` 或 CSS `aspect-ratio` 可以让浏览器提前预留空间。

首屏大图还常常是 LCP 元素，应关注：

- 是否被懒加载误伤。
- 是否需要 `preload` 或 `fetchpriority="high"`。
- 是否尺寸过大。
- 是否格式合适，例如 WebP/AVIF。
...
- 字体未加载时，浏览器可能先隐藏文字或用 fallback 字体。
- 字体加载完成后，文字宽高可能变化，引发布局移动。

可用 `font-display` 控制策略：
...
`font-display: swap` 会先用备用字体显示，字体加载后再替换，能减少空白文本时间，但可能带来字体切换引发的轻微布局变化。

### 3.11 增量渲染：为什么不是每次都走完整流程

浏览器渲染流程最重要的性能模型是：不同修改触发不同成本。

| 修改类型 | 可能触发 | 例子 |
| --- | --- | --- |
| 改变布局 | Style + Layout + Paint + Composite | `width`、`height`、`margin`、插入 DOM |
| 改变视觉但不改布局 | Style + Paint + Composite | `color`、`background`、`box-shadow` |
| 改变合成属性 | Style + Composite | `transform`、`opacity` |
...
Layout 变了
  ↓
后面 Paint、Raster、Composite 往往都受影响

Paint 变了
  ↓
不一定需要重新 Layout，但要重新绘制和合成
...
这也是“回流一定伴随重绘，重绘不一定伴随回流”的来源。但更现代、更完整的说法是：

- Layout 变化通常会导致后续 Paint/Composite。
- Paint 变化不一定导致 Layout。
- Composite 变化可能跳过 Layout 和 Paint。

### 3.12 实际例子：首屏页面如何一步步显示
...
<html>
  ...
  <body>
    <h1>商城首页</h1>
    <img class="hero" src="/hero.webp" width="1200" height="600" alt="">
  </body>
</html>
```
...
```text
1. Network Service 下载 HTML
   ↓
2. Renderer 边接收边解析 HTML
   ↓
3. 发现 style.css，发起 CSS 请求
   ↓
4. 发现 defer main.js，发起 JS 请求，但不立即阻塞 DOM 解析
   ↓
5. 继续解析 body，构建 h1、img DOM
   ↓
6. 发现 hero 图片，发起图片请求
   ↓
7. CSS 下载完成，构建 CSSOM
...
8. DOM + CSSOM 进行样式计算
   ↓
9. Layout：根据 h1、图片预设宽高计算位置
   ↓
10. Paint：生成文字、图片占位、背景等绘制指令
   ↓
11. Raster + Composite：输出首帧
   ↓
12. defer 脚本在 DOM 解析完成后执行
   ↓
13. hero 图片解码完成，更新对应区域
```

如果图片没有设置尺寸：

```html
<img class="hero" src="/hero.webp" alt="">
```

浏览器一开始不知道图片占多大，图片加载完成后可能重新 Layout，把下面内容挤开，造成 CLS。
...
| 概念 | 作用 | 是否直接显示 |
| --- | --- | --- |
| DOM | 表示文档结构，供 JS 操作 | 不直接显示 |
| CSSOM | 表示样式规则，供样式计算使用 | 不直接显示 |
| Render Tree | 表示需要渲染的可见对象及样式 | 用于后续 Layout/Paint |

#### display none VS visibility hidden VS opacity 0

| 写法 | 是否占据布局 | 是否绘制可见 | 常见影响 |
| --- | --- | --- | --- |
| `display: none` | 不占据 | 不绘制 | 会影响布局 |
| `visibility: hidden` | 占据 | 不可见 | 通常不影响布局，但影响绘制 |
| `opacity: 0` | 占据 | 完全透明 | 可能仍可点击，可能参与合成 |

#### Reflow VS Repaint VS Composite

| 概念 | 含义 | 例子 |
| --- | --- | --- |
| Reflow / Layout | 重新计算位置和尺寸 | 改 `width`、插入 DOM |
| Repaint / Paint | 重新生成绘制指令 | 改 `background`、`color` |
| Composite | 合成图层生成最终帧 | 改 `transform`、`opacity` |

#### async VS defer 对渲染的影响

| 属性 | 下载 | 执行时机 | 是否阻塞 HTML 解析 |
| --- | --- | --- | --- |
| 无属性 | 遇到就下载并执行 | 立即执行 | 阻塞 |
| `defer` | 并行下载 | DOM 解析完成后，DOMContentLoaded 前按顺序执行 | 不阻塞解析 |
| `async` | 并行下载 | 下载完成后尽快执行，顺序不保证 | 执行时会打断解析 |

`defer` 适合依赖 DOM、需要保持顺序的业务脚本；`async` 适合独立脚本，例如统计、广告、第三方 SDK。

### 3.14 高频面试题

| 问题 | 回答思路 |
| --- | --- |
| 浏览器渲染流程是什么？ | 从 HTML 解析 DOM、CSS 解析 CSSOM、样式计算、Layout、Paint、Layer、Raster、Composite 到上屏。强调真实浏览器会边解析边渲染，并支持增量更新。 |
| DOM 和 Render Tree 有什么区别？ | DOM 是文档结构，Render Tree 是可渲染对象结构。`display: none`、`head`、`script` 不生成可见渲染对象，伪元素可能生成渲染内容。 |
| CSS 会阻塞 DOM 解析吗？ | CSS 不直接阻塞 DOM 解析，但阻塞渲染；如果后续同步 JS 可能读取样式，JS 可能等待 CSSOM，进而间接阻塞 HTML 解析。 |
| JS 为什么会阻塞 HTML 解析？ | 同步 JS 可能读写 DOM、调用 `document.write`、读取布局。浏览器必须保证脚本看到的 DOM 状态和执行顺序符合规范。 |
| Layout、Paint、Composite 有什么区别？ | Layout 算几何位置，Paint 生成绘制指令，Composite 合成图层上屏。Layout 成本通常最高，Composite 通常较低。 |
| 为什么 `transform` 动画比 `left` 更推荐？ | `left` 可能触发布局和绘制；`transform` 通常只改变合成参数，可以复用图层纹理，减少主线程压力。 |
| 什么是强制同步布局？ | JS 写入影响布局的属性后立刻读取 `offsetWidth` 等布局信息，浏览器被迫提前 Layout。多次交替读写会造成 layout thrashing。 |
| 图片为什么要设置宽高？ | 浏览器可提前预留布局空间，避免图片加载完成后撑开页面造成 CLS，也有利于稳定首屏布局。 |
| 为什么 CSS 放头部、JS 放底部或加 defer？ | CSS 尽早加载可尽快得到首屏样式；同步 JS 会阻塞解析，放底部或使用 `defer` 可以减少对 DOM 构建和首屏渲染的影响。 |
| 首屏性能和渲染流程有什么关系？ | FCP 受关键 CSS、HTML 解析、主线程任务影响；LCP 常受首屏大图、字体、CSS、JS 影响；CLS 常由图片无尺寸、字体切换、动态插入内容导致。 |

### 3.15 本章总结

渲染流程的核心不是背名词，而是理解每一阶段为什么存在：
...
Layout 解决盒子位置和尺寸问题
  ↓
Paint 解决画什么、按什么顺序画的问题
  ↓
Raster 解决绘制指令到位图的问题
  ↓
Composite 解决多图层如何合成最终帧的问题
```

性能优化也围绕这条链路展开：

- 少阻塞：减少关键 CSS 和同步 JS。
- 少布局：避免频繁 DOM 读写和强制同步布局。
- 少绘制：避免大面积复杂重绘。
- 多走合成：动画优先使用 `transform`、`opacity`。
- 稳定布局：图片和广告位提前预留尺寸，减少 CLS。

面试时最稳的回答方式是：先讲完整主链路，再解释 CSS、JS、图片、字体分别影响哪个阶段，最后落到 Layout/Paint/Composite 的性能差异和实际优化手段。
...
因为浏览器自动携带 Cookie 的策略是只看**请求目标**是谁？如果有请求目标的 Cookie 会自动携带，不看这个请求是谁发送的。

> CSRF 怎么防御

- CSRF Token 防御：意思是服务端不止生成 Cookie 还有 CSRF Token；因为浏览器的同源策略，禁止读取非同源站点的响应，虽然会自动携带 Cookie，但是拿不到 CSRF Token

- SameSite Cookie：现代浏览器默认给 `Cookie` 设置 `SameSite = Lax`；意思是跨域请求默认不携带 Cookie；`SameSite` 有三个属性值 `Strict` 完全不携带、`Lax` 大部分不携带

- 检查 Origin / Referer：在服务端检查请求来源，如果为未知来源则返回403
### 15.4 工作流程
