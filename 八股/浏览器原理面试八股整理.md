...
现代浏览器不是一个单体程序，而是由多个进程协作完成网页加载、渲染、脚本执行、网络请求、插件、GPU 合成等工作。以 Chromium 为例，常见进程包括 Browser 进程、Renderer 进程、GPU 进程、Network Service 进程、Plugin/Utility 进程等。

面试可以这样答：

> 浏览器采用多进程架构，是为了提高稳定性、安全性和性能隔离。Browser 进程负责主控、地址栏、标签页管理、权限、下载等；Renderer 进程负责页面解析、JS 执行、布局和绘制；GPU 进程负责图层合成和硬件加速；Network 进程负责网络请求。某个页面崩溃时，通常不会拖垮整个浏览器。

### 1.2 底层原理

浏览器多进程的核心设计目标是隔离：

- 稳定性隔离：一个标签页崩溃，不影响其他标签页和浏览器主进程。
- 安全隔离：网页运行在沙箱 Renderer 中，减少恶意页面直接访问系统资源的能力。
- 性能隔离：不同页面、站点、插件、GPU 任务可以独立调度。
- 权限隔离：敏感能力由 Browser 进程统一控制，Renderer 不能随意读写文件、访问摄像头或操作系统能力。

为什么不全部放在一个进程？单进程内存更省、通信更简单，但一个页面死循环或崩溃就可能拖垮整个浏览器；恶意脚本也更容易突破边界。现代 Web 越来越复杂，多进程是用资源换可靠性和安全性。
...
1. 用户在地址栏输入 URL。
2. Browser 进程接收导航请求。
3. Network 进程发起 DNS、TCP/TLS、HTTP 请求。
4. Browser 进程根据站点隔离策略选择或创建 Renderer 进程。
5. Renderer 进程解析 HTML、CSS，执行 JS，生成渲染结果。
6. Renderer 把绘制指令或图层信息提交给 GPU 进程。
7. GPU 进程合成最终画面并显示。
...
flowchart TD
    ...
    B --> C[Network 进程请求资源]
    C --> B
    B --> D[创建或复用 Renderer 进程]
    D --> E[解析 HTML/CSS 执行 JS]
    E --> F[布局、绘制、生成图层]
    F --> G[GPU 进程合成]
    G --> H[屏幕显示]
```
...
| 问题 | 标准回答 | 继续追问 |
| --- | --- | --- |
| 浏览器为什么是多进程？ | 为了稳定性、安全性、性能隔离和权限隔离。 | 多进程有什么缺点？ |
| 一个 Tab 一个进程吗？ | 不一定。现代浏览器通常结合站点隔离、iframe、进程复用等策略。 | iframe 会单独进程吗？ |
| Renderer 进程能直接访问文件吗？ | 不能，Renderer 运行在沙箱里，敏感操作要通过 Browser 进程。 | 为什么要沙箱？ |
| GPU 进程的作用？ | 负责图层合成、硬件加速、部分栅格化，提高渲染性能。 | GPU 加速一定更快吗？ |

### 1.5 常见追问

Q：多进程有什么代价？

A：内存占用更高，进程间通信 IPC 有开销，架构更复杂。浏览器会通过进程复用、冻结后台页面、内存回收等策略控制资源消耗。

Q：站点隔离是什么？

A：站点隔离是把不同站点放到不同 Renderer 进程中，即使存在漏洞，也尽量避免一个站点读取另一个站点的数据。它是浏览器安全模型的重要强化。

### 1.6 实际项目场景

- 一个页面占用内存过高或 JS 死循环，会导致当前 Tab 卡死甚至崩溃，但通常不会影响整个浏览器。
- 多 iframe 的复杂后台系统，要注意内存和进程开销。
- 性能排查时，Chrome Task Manager 可以看到不同页面、扩展、GPU、Network 的资源占用。
...
进程是资源隔离单位，线程是执行任务的单位。Renderer 进程内部通常有主线程、合成线程、光栅线程、Worker 线程等。前端最常接触的是主线程，因为 JS 执行、DOM 操作、样式计算、布局大多发生在主线程。

面试回答：

> 浏览器渲染进程中，主线程负责解析 HTML、执行 JS、计算样式、布局和部分绘制任务；合成线程负责图层合成；Worker 线程用于后台计算。JS 和 DOM 运行在同一主线程，是为了避免多线程同时修改 DOM 带来的复杂同步问题。

### 2.2 底层原理

JS 能操作 DOM，如果 JS 执行和渲染布局完全并行，就会出现竞态：渲染线程正在读取 DOM，JS 同时修改 DOM，结果不确定。浏览器选择让 JS 执行和 DOM 更新集中在主线程，以牺牲部分并行性换取一致性和简单模型。

这也是为什么长 JS 任务会造成页面卡顿。主线程被占用时，用户输入、动画、布局、绘制都要排队。
...
1. 用户点击按钮。
2. Browser 进程把输入事件发送给 Renderer。
3. 主线程执行事件回调。
4. JS 修改状态或 DOM。
5. 浏览器重新计算样式、布局、绘制或合成。
6. 画面更新。
...
| 问题 | 标准回答 | 继续追问 |
| --- | --- | --- |
| JS 为什么是单线程？ | 为了避免多线程同时操作 DOM 造成同步复杂度和竞态问题。 | Worker 为什么不能操作 DOM？ |
| 浏览器有哪些线程？ | 主线程、合成线程、光栅线程、Worker 线程、网络相关线程等。 | 定时器在哪个线程计时？ |
| 页面卡顿本质是什么？ | 主线程长时间被占用，无法及时响应输入和渲染。 | 怎么定位 Long Task？ |

### 2.5 常见追问

Q：定时器回调是定时器线程执行的吗？

A：不是。定时器线程或浏览器内部机制负责计时，时间到后把回调放入任务队列，最终仍由 JS 主线程执行。

Q：CSS 动画为什么有时不卡？

A：如果动画只影响 `transform`、`opacity` 等合成属性，可以在合成线程/GPU 路径上完成，减少主线程布局和绘制压力。

### 2.6 实际项目场景

- React 页面卡顿常见原因是主线程执行了大组件树渲染、大量同步计算或频繁 DOM 读写。
- 大数据处理、图片处理、加密压缩可放到 Web Worker，避免阻塞主线程。
- 动画优先使用 `transform` 和 `opacity`，避免频繁修改 `width`、`top`、`left`。
...
XSS 跨站脚本攻击，本质上是利用浏览器会自动执行 JS ， 攻击者把恶意 JavaScript 注入到网页中，让其他用户浏览页面时执行。

CSRF 跨站请求伪造，本质上是利用浏览器发送请求时自动携带 cookie，是诱导用户浏览器带着登录态发起非预期请求。
