...
**Fiber 与调度**：
- Fiber 是 React 的可中断工作单元，也是链表结构的虚拟调用栈。
- Render 阶段可以被打断、恢复、丢弃；Commit 阶段必须同步完成，因为会真实修改 DOM。
- React 18 之后的并发能力重点是让高优先级更新先响应，例如输入、点击优先于低优先级渲染。

**setState 与批处理**：
- React 18 自动批处理覆盖 Promise、setTimeout、原生事件等更多异步场景。
- 多次 `setCount(count + 1)` 使用同一闭包值，可能只加一次；函数式更新 `setCount(c => c + 1)` 基于最新队列值。
- `flushSync` 可以强制同步刷新，但会牺牲调度收益，谨慎使用。

**Hooks 追问**：
- Hooks 依赖调用顺序保存状态，所以不能放在条件、循环和普通函数里。
- `useEffect` 在浏览器绘制后执行，`useLayoutEffect` 在 DOM 更新后、绘制前同步执行。
- 依赖数组不是优化开关，而是声明 effect 使用了哪些响应式值。
- `useMemo/useCallback` 不是越多越好，它们本身也有比较和缓存成本。

**性能优化追问**：
- 先定位瓶颈，再优化：React DevTools Profiler 看组件提交耗时和渲染原因。
- 常见手段：状态下沉或上移、拆分组件、稳定引用、虚拟列表、懒加载、减少 Context 大范围更新。
- `key` 的本质是帮助 React 在同级节点中匹配旧 Fiber，不能用随机数，列表可变时不要用 index。

**状态管理追问**：
- Redux 适合可预测、可追踪、跨页面共享的复杂状态；不是所有状态都应该进全局 store。
- React Query/SWR 适合服务端状态：缓存、重试、失效、重新拉取。
- Zustand/Jotai/Recoil 更偏轻量本地或原子状态，要根据团队和复杂度选型。

### 2. 前端/全栈工程师需要补齐的 React 能力

**服务端组件与客户端边界**：
- Server Components 适合数据读取、减少客户端 JS、保护服务端密钥。
- Client Components 适合交互、浏览器 API、状态和 effect。
- 边界设计要避免把大组件树都标成 `'use client'`，否则会丢失服务端组件收益。

**数据请求架构**：
- 客户端请求关注缓存、重试、取消、乐观更新和错误恢复。
- SSR/SSG/RSC 请求关注鉴权、缓存隔离、请求去重、流式渲染和 SEO。
- 表单提交通常要考虑 loading、重复提交、服务端校验、错误回填、乐观 UI。
...
优秀组件不仅能渲染，还要考虑受控/非受控、可访问性、键盘操作、loading/empty/error 状态、主题和测试。

**测试与质量**：
- 组件测试更推荐从用户行为出发：点击、输入、可见文本、可访问角色。
- 不要过度测试内部实现，例如 state 名称、私有函数调用次数。
- 关键页面要有 E2E 覆盖登录、提交、支付、权限等核心流程。

**全栈协作**：
- React 页面要理解接口分页、排序、筛选、权限字段、错误码和幂等。
- 管理后台类项目要关注表格性能、批量操作、权限控制、审计日志和导入导出。
- C 端项目要关注首屏、SEO、埋点、A/B 实验、灰度和可访问性。

### 3. 回答模板

React 题建议按“现象 -> 原理 -> 场景 -> 取舍”回答。例如 `useMemo`：

1. 现象：缓存计算结果，依赖不变时复用。
2. 原理：React 在 Hook 链表中保存上次依赖和结果。
3. 场景：昂贵计算、稳定传给 memo 子组件的引用。
4. 取舍：依赖比较和缓存有成本，简单计算不需要滥用。
