...
- EPSILON 是 JavaScript 能识别的最小误差单位，当误差小于这个单位表示没有误差

- isEqual 表示 **不是判断“完全相等”，而是判断“足够接近”**
...
### 2.1 什么是作用域？什么是作用域链？JavaScript有哪些作用域？
**考点**：变量可见性
...
**闭包定义**：函数能够访问其**词法作用域**外的变量

**词法作用域：** 变量的作用域在代码书写时就被固定，而不是执行阶段
...
**继承** 是一种让一个对象（或类）能够使用另一个对象（或类）的属性和方法，从而实现**代码复用**和**层次结构构建**的机制。
...
JavaScript 是单线程的（为了避免操作 DOM 时的复杂竞态条件）。如果没有事件循环，当程序发起一个网络请求或设置一个 5 秒的定时器时，主线程就会被迫停下来等待结果。

- 后果：网页会卡死，用户无法点击、滚动或进行任何操作。

- 有了事件循环：主线程可以先把耗时任务交给浏览器后台处理，自己继续执行后面的代码，等结果回来了再回头处理。

**实现非阻塞 I/O (Non-blocking I/O)**

在现代 Web 应用中，I/O 操作（输入/输出）极其频繁。事件循环允许 JavaScript 在等待 I/O 结果的同时，继续响应用户的其他交互。这种“异步非阻塞”模型使得单线程的 JavaScript 能够处理高并发的逻辑。

**任务优先级的管理**

通过区分微任务和宏任务，事件循环允许开发者精细化控制代码的执行顺序。

- 微任务（如 Promise）通常用于需要立即响应的状态更新。

- 宏任务（如 setTimeout）则用于可以稍微延后的、较重的工作。
