...
### 0.1 ANSI/VT escape sequences

后端的 Linux 服务器无法直接控制你浏览器的 CSS 样式。它要想让前端的某个字变红、让光标跳到第一行、或者清空屏幕，就必须把这些“指令”伪装成特殊的特殊文本，混在普通字符里发给前端。

> 这些特殊文本就是 ANSI/VT 转义序列。

而 `xterm.js` 的核心工作，就是充当一个“翻译官”，把这些文本指令解析出来，变成网页上的视觉效果。

#### 0.1.1 如何运行的？

`xterm.js` 内部有一个非常高效的**状态机（State Machine）**

当你调用 `term.write(data)` 时，数据会通过这个状态机：
```
正常状态: 打印文本
...
- 在默认情况下，`xterm.js` 无法感知外层 HTML 容器（比如一个 `<div>`）的大小变化。如果外层容器变大了，终端并不会自动铺满，而是留下一片空白；如果容器变小了，终端内容就会被截断。
- `fitAddon.fit()` 的核心作用就是：**让终端的网格矩阵，去填满它的宿主 HTML 容器。**
- 它通过计算宿主容器当前的像素宽高，换算出在当前字体大小下，容器最多能容纳多少**列（Cols）** 和多少 **行（Rows）**，然后自动调用 `terminal.resize(cols, rows)` 来调整终端网格。
