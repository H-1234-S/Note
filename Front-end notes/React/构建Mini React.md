...
#### Q1：在 `WorkLoop` 中，为什么需要 `deadline.timeRemaining()`？React 又是如何实现“可中断渲染”的？

- **答案要点**：
    
    - **协作式调度**：浏览器每一帧（约 16.6ms）除了执行 JS，还要处理样式计算、布局和绘制。React 为了不阻塞 UI，只在浏览器空闲时执行任务。
        
    - **中断时机**：React 将渲染拆分为微小的 **Fiber 单元**。每执行完一个 `performUnitOfWork`（即处理完一个 Fiber 节点），都会回到循环处检查时间。
        
    - **不可中断性**：一旦进入 `performUnitOfWork` 内部，该节点的逻辑必须同步执行完。中断只发生在**节点与节点之间**。
        

---

### 🟡 第二板块：调和阶段（Reconciliation）

#### Q2：在 `reconcileChildren` 时，`alternate` 属性的作用是什么？它是何时挂载到 Fiber 上的？

- **答案要点**：
    
    - **作用**：`alternate` 是连接“当前屏幕树（Current Tree）”与“正在构建树（WIP Tree）”的桥梁，用于复用旧的状态和 DOM 节点。
        
    - **挂载时机**：在**父节点**调和子节点时。父节点会对比 `element` 和 `oldFiber`，如果类型相同，则创建 `newFiber` 并令其 `alternate = oldFiber`。
        

#### Q3：如果 `[A, B, C]` 变为 `[C, A, B]`，不带 `key` 的 Diff 会发生什么？

- **答案要点**：
    
    - **原地复用**：由于按索引对比，React 会认为第一个位置的 A 变成了 C，执行 `UPDATE` 更新属性；第二个位置的 B 变成了 A，以此类推。
        
    - **开销与副作用**：产生大量不必要的 DOM 属性操作。更严重的是，如果节点内有非受控状态（如 input 焦点），会因为 DOM 没移动而导致 UI 状态串位。
...
#### Q4：为什么 Hooks 的存储依赖执行顺序（Index）？如果在 `if` 语句中使用会怎样？

- **答案要点**：
    
    - **设计权衡**：不使用字符串 Key 是为了避免命名冲突和简化代码。React 通过全局 `hookIndex` 自动关联新旧 Hook。
        
    - **串位灾难**：如果 `if` 跳过了第 1 个 Hook，第 2 个 Hook 在执行时，其 `hookIndex` 会指向旧 Fiber 的第 1 个 Hook 存储位置，导致取错状态。
        

#### Q5：你的 `useState` 是如何支持连续调用 `setCount(c => c + 1)` 的？

- **答案要点**：
    
    - **更新队列**：每个 Hook 都有一个 `queue`。
        
    - **批量处理**：在下一次渲染执行函数体时，会遍历 `oldHook.queue` 中的所有 `action`，将其连续作用于 `state`，最终计算出最新的状态值。
...
#### Q6：为什么 `commit` 阶段必须是同步不可中断的？

- **答案要点**：
    
    - **UI 一致性**：如果在修改真实 DOM 时中断，用户可能会看到“渲染了一半”的界面（例如只有文字没有背景），造成闪烁和体验破碎。
        

#### Q7：为什么删除节点需要 `commitDeletion` 递归寻找？直接 `removeChild` 不行吗？

- **答案要点**：
    
    - **逻辑与物理结构不一致**：Fiber 树中有函数组件节点（无 DOM）。如果要删除一个函数组件，必须向下递归直到找到它 return 的第一个**含有真实 DOM** 的子节点才能执行删除操作。