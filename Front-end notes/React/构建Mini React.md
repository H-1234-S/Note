...
---
# 复盘
### 1. 虚拟 DOM (Virtual DOM) —— “一份轻量级的蓝图”

**是什么：**

虚拟 DOM 本质上是一个普通的 **JavaScript 对象**，它是对真实 DOM 的抽象描述。

在你的笔记中，`createElement` 函数返回的那个对象（包含 `type` 和 `props`）就是虚拟 DOM。

**解决了什么问题：**

- **性能优化**：直接操作真实 DOM 是非常昂贵的（会触发重绘和回流）。通过在 JS 层操作对象，我们可以先计算出变化，再批量更新 DOM。
    
- **跨平台**：有了这层抽象，React 既可以渲染到浏览器（ReactDOM），也可以渲染到移动端（React Native）。
...
Diff 算法是 React 用来**对比新旧两棵虚拟 DOM 树**，找出其中差异的具体过程。

**核心逻辑（基于你的笔记实现）：**

- **按层对比**：React 只会对比同一层级的节点，不会跨层级比较。
    
- **类型检查**：如果 `type` 相同，就复用节点并只更新属性（`UPDATE`）；如果 `type` 不同，直接删除旧的，创建新的（`PLACEMENT`）。
    
- **Key 的作用**：`key` 是节点的唯一标识。有了 `key`，Diff 就能发现节点只是“换了位置”，从而通过移动 DOM 来代替销毁重建，极大地提升性能。
...
Fiber 既是一种**数据结构**，也是一种**调度算法**。

- **作为数据结构**：它是一个增强版的虚拟 DOM。除了 `type` 和 `props`，它还有 `child`（指向第一个子节点）、`sibling`（指向下一个兄弟节点）和 `return`（指向父节点）等指针。这让它变成了一个**链表结构**。
    
- **作为调度算法**：它允许 React 将庞大的渲染任务拆分成一个个微小的“工作单元”。
    

**解决了什么问题：**

在 Fiber 出现之前（Stack Reconciler），React 的更新是同步且不可中断的。如果组件树很深，主线程会被卡死，导致页面掉帧、输入无响应。

**Fiber 让渲染变得“可中断”**：

1. **Render 阶段**：利用 `requestIdleCallback` 在浏览器空闲时遍历 Fiber 链表。如果时间不够了（`deadline.timeRemaining() < 1`），就停下来，把控制权还给浏览器。
    
2. **Commit 阶段**：当所有 Fiber 节点的差异计算完毕后，一次性同步更新到真实 DOM 上，保证 UI 的一致性。
...
我们可以用一个**“装修房子”**的例子串联起来：

1. **虚拟 DOM** 是**设计图纸**。你想改动装修时，先在图纸上改（JS 运算），而不是直接拆墙。
    
2. **Diff 算法** 是**对比新旧图纸的差额表**。它告诉你：“客厅灯换个颜色，厨房墙不用拆，卧室加个柜子”。
    
3. **Fiber** 是**施工调度计划**。以前是一个施工队一口气干完，天塌了也不停，导致你进不去屋子（页面卡顿）。现在改为“按工序拆分”，工人每干完一小块活（一个 Fiber 节点），就停下来问问你：“主人，你要进屋吗？”。如果你要进，他就先停工让你进去（处理高优先级任务），等你走了他再接着干。
    

**总结：**

- **虚拟 DOM** 保证了开发者的声明式编程体验。
    
- **Diff** 保证了更新的最小化。
    
- **Fiber** 保证了在大规模更新时的流畅度（不掉帧）。
